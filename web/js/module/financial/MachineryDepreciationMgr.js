...
var MachineryDepreciationGrid = null;
var MachineryDepreciationQueryGrid = null;

MachineryDepreciationQueryGrid = new QueryGrid(
		"MachineryDepreciationForm",
		[ {name:'name',type:'text',text:'名称'},
		  {name:'type',type:'hidden',text:'',value:'22'}
...
//		                                         ] }
		 ...
		 "addFile",
		 "removeFile",
		 null,//"updateMachineryDepreciation",
		 "pages/module/financial/MachineryDepreciation/MachineryDepreciationDialog.html",
		 "MachineryDepreciationDialogForm"
);

MachineryDepreciationGrid = $("#MachineryDepreciationGrid").jqGrid(
	{	
		...
		colModel:[ 
		 ...
		 rowList:[10,50,100],
		 pager: '#MachineryDepreciationPager', 
		 //sortname: 'id',
		 viewrecords: true,
		 //sortorder: "desc", 
		 caption:"应收款",
...
jQuery("#MachineryDepreciationGrid").jqGrid('navGrid','#MachineryDepreciationPager',{edit:false,add:false,del:false});

MachineryDepreciationQueryGrid.setGrid(MachineryDepreciationGrid);

//MachineryDepreciationQueryGrid.createDoubleClick();

//var d = FormUtil.getValue("MachineryDepreciationForm");
//alert(d["type"]);
//MachineryDepreciationQueryGrid.query();

MachineryDepreciationQueryGrid.afterReset = function() {
	FormUtil.setValue("MachineryDepreciationForm",{type:"22"});
};

MachineryDepreciationQueryGrid.afterPopup = function() {
...
MachineryDepreciationQueryGrid.beforeSave = function() {
...
function downloadMachineryDepreciation() {
	var selectedId = MachineryDepreciationGrid.jqGrid("getGridParam", "selrow");
	if(selectedId == null)
	{
		art.dialog("请选择一条记录");
		return;
	...
	var d = MachineryDepreciationGrid.jqGrid("getRowData", selectedId);
	
	var exname = getExpandingName(d["path"]);
	
	new DownloadComponent().createDownloadComponent("MachineryDepreciationDownloadForm","downloadFiles",d["name"]+"."+exname,d["id"]);
}

function previewMachineryDepreciationFile()
{
	var selectedId = MachineryDepreciationGrid.jqGrid("getGridParam", "selrow");
	if(selectedId == null)
	{
