...
var PayrollGrid = null;
var PayrollQueryGrid = null;

PayrollQueryGrid = new QueryGrid(
		"PayrollForm",
		[ {name:'name',type:'text',text:'名称'},
		  {name:'type',type:'hidden',text:'',value:'17'}
...
//		                                         ] }
		 ...
		 "removeFile",
		 null,//"updatePayroll",
		 "pages/module/financial/Payroll/PayrollDialog.html",
		 "PayrollDialogForm"
);

PayrollGrid = $("#PayrollGrid").jqGrid(
	{	
...
jQuery("#PayrollGrid").jqGrid('navGrid','#PayrollPager',{edit:false,add:false,del:false});

PayrollQueryGrid.setGrid(PayrollGrid);

//PayrollQueryGrid.createDoubleClick();

//var d = FormUtil.getValue("PayrollForm");
//alert(d["type"]);
//PayrollQueryGrid.query();

PayrollQueryGrid.afterReset = function() {
	FormUtil.setValue("PayrollForm",{type:"17"});
};

PayrollQueryGrid.afterPopup = function() {
...
PayrollQueryGrid.beforeSave = function() {
...
function downloadPayroll() {
	var selectedId = PayrollGrid.jqGrid("getGridParam", "selrow");
	if(selectedId == null)
	{
		art.dialog("请选择一条记录");
		return;
	...
	var d = PayrollGrid.jqGrid("getRowData", selectedId);
	
	var exname = getExpandingName(d["path"]);
	
	new DownloadComponent().createDownloadComponent("PayrollDownloadForm","downloadFiles",d["name"]+"."+exname,d["id"]);
}

function previewPayrollFile()
{
	var selectedId = PayrollGrid.jqGrid("getGridParam", "selrow");
	if(selectedId == null)
	{
