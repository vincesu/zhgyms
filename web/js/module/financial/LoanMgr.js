...
var LoanGrid = null;
var LoanQueryGrid = null;

LoanQueryGrid = new QueryGrid(
		"LoanForm",
		[ {name:'name',type:'text',text:'名称'},
		  {name:'type',type:'hidden',text:'',value:'18'}
...
//		                                         ] }
		 ...
		 "removeFile",
		 null,//"updateLoan",
		 "pages/module/financial/Loan/LoanDialog.html",
		 "LoanDialogForm"
);

LoanGrid = $("#LoanGrid").jqGrid(
	{	
...
jQuery("#LoanGrid").jqGrid('navGrid','#LoanPager',{edit:false,add:false,del:false});

LoanQueryGrid.setGrid(LoanGrid);

//LoanQueryGrid.createDoubleClick();

//var d = FormUtil.getValue("LoanForm");
//alert(d["type"]);
//LoanQueryGrid.query();

LoanQueryGrid.afterReset = function() {
	FormUtil.setValue("LoanForm",{type:"18"});
};

LoanQueryGrid.afterPopup = function() {
...
LoanQueryGrid.beforeSave = function() {
...
function downloadLoan() {
	var selectedId = LoanGrid.jqGrid("getGridParam", "selrow");
	if(selectedId == null)
	{
		art.dialog("请选择一条记录");
		return;
	...
	var d = LoanGrid.jqGrid("getRowData", selectedId);
	
	var exname = getExpandingName(d["path"]);
	
	new DownloadComponent().createDownloadComponent("LoanDownloadForm","downloadFiles",d["name"]+"."+exname,d["id"]);
}

function previewLoanFile()
{
	var selectedId = LoanGrid.jqGrid("getGridParam", "selrow");
	if(selectedId == null)
	{
