...
var GeneralLedgerGrid = null;
var GeneralLedgerQueryGrid = null;

GeneralLedgerQueryGrid = new QueryGrid(
		"GeneralLedgerForm",
		[ {name:'name',type:'text',text:'名称'},
		  {name:'type',type:'hidden',text:'',value:'23'}
...
//		                                         ] }
		 ...
		 "addFile",
		 "removeFile",
		 null,//"updateGeneralLedger",
		 "pages/module/financial/GeneralLedger/GeneralLedgerDialog.html",
		 "GeneralLedgerDialogForm"
);

GeneralLedgerGrid = $("#GeneralLedgerGrid").jqGrid(
	{	
...
jQuery("#GeneralLedgerGrid").jqGrid('navGrid','#GeneralLedgerPager',{edit:false,add:false,del:false});

GeneralLedgerQueryGrid.setGrid(GeneralLedgerGrid);

//GeneralLedgerQueryGrid.createDoubleClick();

//var d = FormUtil.getValue("GeneralLedgerForm");
//alert(d["type"]);
//GeneralLedgerQueryGrid.query();

GeneralLedgerQueryGrid.afterReset = function() {
	FormUtil.setValue("GeneralLedgerForm",{type:"23"});
};

GeneralLedgerQueryGrid.afterPopup = function() {
...
GeneralLedgerQueryGrid.beforeSave = function() {
...
function downloadGeneralLedger() {
	var selectedId = GeneralLedgerGrid.jqGrid("getGridParam", "selrow");
	if(selectedId == null)
	{
		art.dialog("请选择一条记录");
		return;
	...
	var d = GeneralLedgerGrid.jqGrid("getRowData", selectedId);
	
	var exname = getExpandingName(d["path"]);
	
	new DownloadComponent().createDownloadComponent("GeneralLedgerDownloadForm","downloadFiles",d["name"]+"."+exname,d["id"]);
}

function previewGeneralLedgerFile()
{
	var selectedId = GeneralLedgerGrid.jqGrid("getGridParam", "selrow");
	if(selectedId == null)
	{
